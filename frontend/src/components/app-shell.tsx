...
const navItems: Array<{ view: View; label: string; icon: LucideIcon; writeOnly?: boolean }> = [
  { view: "dashboard", label: "Painel", icon: LayoutDashboard },
  { view: "calendar", label: "Cronograma", icon: CalendarDays },
  { view: "manage", label: "Gestão", icon: Edit3, writeOnly: true },
  { view: "reminders", label: "Lembretes", icon: Bell },
  { view: "sync", label: "Sincronização", icon: RefreshCw },
];

export function visibleNavItems(canWrite: boolean) {
  return navItems.filter((item) => !item.writeOnly || canWrite);
}
...
function NavButtons({
  ...
  return (
    <nav className="grid gap-1">
      {visibleNavItems(canWrite).map((item) => (
        <button
          key={item.view}
          className={cx(
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
        <span className="rounded-lg bg-white px-2 py-1.5 text-center font-medium text-slate-600">
          {canWrite ? "Pode editar" : "Somente leitura"}
        </span>
        <span className="rounded-lg bg-white px-2 py-1.5 text-center font-medium text-slate-600">
          {lastLoadedAt ? `Atualizado ${formatTime(lastLoadedAt.toISOString())}` : "Aguardando"}
        </span>
      </div>
    </div>
  );
  ...
  return (
    <main className="min-h-screen bg-app text-slate-950">
      ...
      {drawerOpen ? (
        ...
        >
          <aside className="flex h-full w-[290px] max-w-[85vw] flex-col gap-4 overflow-y-auto bg-white p-4 shadow-2xl animate-drawer-in">
            ...
            <NavButtons activeView={activeView} canWrite={canWrite} onNavigate={navigate} />
      ...
      <div className="mx-auto grid max-w-[1500px] gap-4 px-3 py-3 sm:px-5 lg:grid-cols-[260px_minmax(0,1fr)] lg:px-6">
        {/* Sidebar (desktop) */}
        <aside className="surface sticky top-3 z-30 hidden h-fit max-h-[calc(100vh-1.5rem)] flex-col gap-4 overflow-y-auto p-3 lg:flex">
